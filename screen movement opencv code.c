...
int i,j,ser;
int adjustx=25,adjusty=6,constx=4,consty=16;
int lower=100,upper=255,count=0,low=0,high=255;
int cnt =0;
...
int main()
{
	ser = serialOpen("/dev/ttyACM0",57600);
	if(ser==-1)
	{
		printf("exiting");
	}	
...
//	cvNamedWindow("Trackbar",1);
  ...
  //
	...
	/*	if(newX>0)
		...
		printf("%d\n",flag);*/
		if(newX<211 && newY<150&& newX>0)
			serialPutchar(ser,'a');
		else if(newX<211 && newY>300)
			serialPutchar(ser,'b');
		else if(newX>422 && newY>300)
			serialPutchar(ser,'c');
		else if(newX>422 && newY<150)
			serialPutchar(ser,'d');
		else if(newX<211 && newX>0)
			 serialPutchar(ser,'q');	
		else if(newX>422)
			 serialPutchar(ser,'s');
		else if(newY<150 && newX>0)
			 serialPutchar(ser,'p');
		else if(newY>300)
			 serialPutchar(ser,'r');
		else if(newX>0)
			serialPutchar(ser,'m');	 
		else serialPutchar(ser,'n');		 	 						  
